...
MODEL_PATH=checkpoints/sft-exp-1
...
echo "timestamp,tensor_parallel_size,batch_size,real_time,user_time,sys_time" > $TIMING_LOG

echo "Starting grid search with $(( ${#TENSOR_PARALLEL_SIZES[@]} * ${#BATCH_SIZES[@]} )) combinations..."
...
for tensor_size in "${TENSOR_PARALLEL_SIZES[@]}"; do
    for batch_size in "${BATCH_SIZES[@]}"; do
        ...
        OUTPUT_FILE="gridsearch_results/output_tp${tensor_size}_bs${batch_size}.parquet"
        
        echo "----------------------------------------"
        echo "Running: tensor_parallel_size=${tensor_size}, batch_size=${batch_size}"
        ...
        if [ -f "$TIME_OUTPUT" ]; then
            ...
            echo "${TIMESTAMP},${tensor_size},${batch_size},${TIMING_DATA}" >> $TIMING_LOG
...
echo ""
echo "=== TIMING SUMMARY ==="
echo "tensor_parallel_size,batch_size,real_time"
tail -n +2 "$TIMING_LOG" | cut -d',' -f2,3,4 | sort -t',' -k3 -n 
